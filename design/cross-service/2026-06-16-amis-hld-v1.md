...
4. **ResearchAsset as First-Class Artifact**: Knowledge survives even when models fail
5. **ExperimentArtifact as First-Class Artifact**: Experiments become searchable, versioned assets
6. **Candidate Registration in Core**: Candidates registered as governance artifacts for lineage
7. **Report Artifacts in Core**: Reports are immutable artifacts for promotion traceability
8. **Research Program Management in Core**: Programs, Tracks, Milestones for R&D control tower
9. **Mandatory Gate 0**: Dependency health blocking with no overrides
10. **Mandatory Regime Governance**: VIX, Volatility, Market Structure, HTF Alignment required
...
**Registry Service**
- Immutable artifact registry (FeatureSchema, LabelVersion, Dataset, ModelArtifact, ResearchAsset, ExperimentArtifact, CandidateArtifact, ReportArtifact)
